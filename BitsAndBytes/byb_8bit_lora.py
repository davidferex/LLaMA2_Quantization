...
tracker1 = OfflineEmissionsTracker(country_iso_code="ESP", allow_multiple_runs = True, output_file= "./emissions_8bit_quant_batch64_ByB_LORA.csv", gpu_ids=[0])
tracker2 = OfflineEmissionsTracker(country_iso_code="ESP", allow_multiple_runs = True, output_file= "./emissions_8bit_eval_batch64_ByB_LORA.csv", gpu_ids=[0])

wandb.init(project="ByB_Lora", name="8bit_batch64")
...
byb_config = BitsAndBytesConfig(load_in_8bit=True)
model = LlamaForSequenceClassification.from_pretrained(
    model_name,
    token=hf_token,
    config=config,
    device_map="auto",
    quantization_config=byb_config)
...
# Mostrar resultados
print(f"Modelo LoRA cuantizado byb 8bit - Tiempo de inferencia: {time_no:.2f}s")
